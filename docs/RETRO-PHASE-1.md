...
`backend/src/lib/schemas.ts` 寫一次，BE Fastify validation + OpenAPI emit + FE 共用 type 都吃同一個。Schema 換欄位時 TS 編譯立刻喊話，FE/BE 不會偷偷 drift。整個 Phase 1 沒有發生過「FE 拿到的 shape 跟 BE 寫的不一樣」這種事 — 除了 [polymorphic 漏網事件](prompts/01-polymorphic-schema-missed.md)，那個是因為 schema 根本沒寫該欄位（option A nullable design 第一輪沒落地）。
...
- PR #148 searchListDTO 漏網 — review 抓到 second DTO drift（[prompt log 01](prompts/01-polymorphic-schema-missed.md)）
...
### 5. Per-workspace Dockerfile（[prompt log 02](prompts/02-railway-dockerfile-pnpm-workspace.md)）
...
### 2. polymorphic schema 只 ship FE（[prompt log 01](prompts/01-polymorphic-schema-missed.md)）
...
Lorem Picsum 圖在 Phase 1 polish 階段加入，CSP `img-src` 第一次用精準 whitelist 還是被 user 看到 blocked。Wildcard 在 nginx config 不知為何被吃掉 + picsum CDN trail 可能變動。第二輪改 `img-src 'self' data: https:` 解決（[prompt log 03](prompts/03-csp-img-src-progressive.md)）。**Lesson**：demo 階段 CSP 該防 inline script / base-uri hijack，不是 image origin。
...
|----|--------|-------|--------|
| A1 | PM 切票要 cross-link FE 元件 + BE schema 為兄弟 issue | PM agent | open（記憶內 [`feedback_pm_scope_sizing`](../../../../.claude/projects/-Users-wuyiheng/memory/) 已記） |
...
| A6 | Cross-agent review 在 multi-DTO drift 場景特別關注 | reviewer agent | open（已在 [prompt log 01](prompts/01-polymorphic-schema-missed.md) 紀錄） |
