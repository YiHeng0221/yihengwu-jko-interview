...
## RR-015 — fix(infra): Dockerfile copy all workspace package.json (fix Railway build)
- PR: #115
- Date: 2026-05-25
...
### Key concerns
- `backend/Dockerfile:14` — 注解區塊延伸至 3 行，違反「one short line max」規範；WHY 本身合理，可濃縮為單行。`frontend/Dockerfile` 同樣有 4 行注解。🟡
- `backend/Dockerfile:1` 與 `frontend/Dockerfile:8` — node image tag 不一致（`node:22-bookworm-slim` vs `node:22.15-bookworm-slim`）；pre-existing，此 diff 範圍外。🟣

### Round history
- Round 1: 2026-05-25 — pass（root cause 正確，修法最小化；runner/builder 最終 stage 隔離效益保留）

---

## RR-013 — feat(web): P1-FE-13 ErrorBoundary + OfflineBanner + a11y polish
- PR: #104
- Date: 2026-05-24
- Reviewer (first): local Claude Code (claude-sonnet-4-6, via `/review`)
- Reviewer (cross-agent): n/a
...
### Key concerns
- `ErrorBoundary.tsx:21` / `routes.tsx` — `ErrorBoundary` 與 `OfflineBanner` 均未掛載至 `main.tsx` / `routes.tsx`，AC-011（捕捉 5xx/4xx-non-401）與 AC-012（顯示離線橫條）runtime 行為完全無法驗證。🔴
- `ErrorBoundary.tsx:5` — `getErrorStatus` 使用 `Object.getOwnPropertyDescriptor` 僅查 own property，prototype chain 上的 `status` 屬性會被跳過，導致 401 誤判為可重試。🟡
- `ErrorBoundary.tsx:22` — `componentDidCatch` 未實作，生產環境錯誤完全靜默，無法接入任何 monitoring（Sentry / console.error）。🟡
- `OfflineBanner.tsx:11` — `role="status"` 已隱含 `aria-live="polite"`，顯式宣告冗余。🟡

### Round history
- Round 1: 2026-05-24 — changes-requested（元件未整合進 app tree，AC 無法驗證）
