...
export function MerchandiseCard({
  ...
  return (
    ...
    >
      ...
      <div className="p-2">
        <div className="line-clamp-2 text-sm font-medium text-text-primary">{title}</div>
        {orgName && (
          <div className="mt-0.5 truncate text-xs text-text-secondary">{orgName}</div>
        )}
        {priceNtd !== null && priceNtd !== undefined && (
          <div className="mt-1 text-sm font-semibold text-brand">
            TWD {priceNtd.toLocaleString()}
          </div>
        )}
      </div>
    </article>
